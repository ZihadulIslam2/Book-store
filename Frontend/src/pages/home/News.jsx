        ...
        {news.map((newsData, index) => (
          <SwiperSlide key={index}>
            <div className="flex flex-col   gap-12 sm:flex-row sm:justify-between items-center ">
              ...
              <div className="py-4">
                <Link>
                  <h3 className="text-lg font-medium hover:to-blue-500 mb-4">
                    {newsData.title}
                  </h3>
                </Link>
                <div className="bg-primary h-[4px] mb-5 w-12 "></div>
                <p className="text-sm text-gray-600"> {newsData.description}</p>
              </div>
              <div className="flex-shrink-0">
                <img src={newsData.image} alt="" className="w-full " />
              </div>
            </div>
          </SwiperSlide>
        ))}
